...
class SessionEditor(tk.Toplevel):
    ...
    def _build(self):
        ...
        mode_txt = "✂ برش شکلی" if self.mode=="crop" else "▭ حذف حاشیه خالی"
        ...
        left_hint = ("برای جابه‌جایی بکشید، برای تغییر اندازه گوشه‌ها را بکشید"
                      if self.mode=="crop" else
                      "برای رسم/جابه‌جایی مستطیل برش بکشید، برای تغییر اندازه گوشه‌ها را بکشید")
        tk.Label(lf, text=f"تصویر اصلی  —  {left_hint}",
        ...
        tk.Label(rf, text="پیش‌نمایش  —  نتیجه این مرحله",
                 font=FS, bg=DARK_BG, fg=TEXT_S).pack(anchor="w")
        ...
        AnimatedButton(btn_row, text="↺  بازنشانی", font=FB,
        ...
        AnimatedButton(btn_row, text="◀  قبلی", font=FB,
                  bg=CARD_BG, fg=TEXT_S, padx=14, pady=9,
                  hover_bg=_lighten(CARD_BG,20), press_bg=_darken(CARD_BG,10),
                  command=self._prev).pack(side="left", padx=(0,8))
        AnimatedButton(btn_row, text="بعدی  ▶", font=FB,
                  bg=CARD_BG, fg=TEXT_S, padx=14, pady=9,
                  hover_bg=_lighten(CARD_BG,20), press_bg=_darken(CARD_BG,10),
    ...
    def _build_crop_controls(self, ctrl):
        ...
        tk.Label(top, text="شکل:", font=FS, bg=PANEL_BG, fg=TEXT_S).pack(side="left", padx=(0,8))
        ...
        _sl(sliders, "عرض شکل  (٪ از تصویر)", self._rx, 10, 100)
        _sl(sliders, "ارتفاع شکل (٪ از تصویر)", self._ry, 10, 100)
    ...
    def _build_trim_controls(self, ctrl):
        ...
        tk.Label(top,
                 text="یک مستطیل دور ناحیه‌ای که می‌خواهید نگه دارید بکشید. "
                      "هر چیزی خارج از آن حذف خواهد شد.",
                 font=FS, bg=PANEL_BG, fg=TEXT_S, justify="left").pack(side="left")

        row = tk.Frame(ctrl, bg=PANEL_BG); row.pack(fill="x", padx=16, pady=(2,4))
        AnimatedButton(row, text="✨ پیشنهاد خودکار برش", font=FS,
                  bg=CARD_BG, fg=ACCENT_L,
                  hover_bg=_lighten(CARD_BG,20), press_bg=_darken(CARD_BG,10),
                  padx=10, pady=4,
                  command=self._auto_suggest_trim).pack(side="left", padx=(0,8))
        tk.Label(row, text="(رنگ پس‌زمینه را به‌طور خودکار تشخیص داده و یک کادر پیشنهاد می‌دهد)",
    ...
    def _draw_left_crop(self, c, ox_px, oy_px):
        ...
        self._coord_lbl.config(
            text=f"شکل: {SHAPE_LABELS[shape]}   جابه‌جایی مرکز: X={ox_pct:+.1f}% Y={oy_pct:+.1f}%   "
                 f"اندازه: {self._rx.get()}% × {self._ry.get()}%")
    ...
    def _draw_left_trim(self, c, ox_px, oy_px):
        if self._trim_rect_disp:
            ...
            self._coord_lbl.config(
                text=f"کادر برش: {int(rw)} × {int(rh)} پیکسل "
                     f"(از {self._orig_pil.width} × {self._orig_pil.height})")
        else:
            self._coord_lbl.config(text="هنوز کادر برشی رسم نشده — روی تصویر بکشید")
    ...
    def _auto_suggest_trim(self):
        ...
        if trimmed.size == self._orig_pil.size:
            ...
            messagebox.showinfo("پیشنهاد خودکار برش",
                "ناحیه‌ای از پس‌زمینه برای حذف پیدا نشد (تصویر از قبل حذف‌حاشیه شده، "
                "یا رنگ پس‌زمینه مطابقت ندارد).")
            return
    ...
    def _save_all(self):
        ...
        for fp in self.files:
            ...
            self._prog_lbl.config(text=f"در حال ذخیره {done+errors}/{total}…", fg=TEXT_S)
            self.update_idletasks()

        self._prog_lbl.config(
            text=f"✔  {done} ذخیره شد،  {errors} خطا  →  {self.out_dir}",
            fg=SUCCESS if errors==0 else WARNING)
        messagebox.showinfo("انجام شد", f"{done}/{total} تصویر در این مسیر ذخیره شد:\n{self.out_dir}")
        if errors == 0:
            self.destroy()
...
class App:
    def __init__(self, root):
        self.root = root
        self.root.title("ابزار تغییر اندازه دسته‌ای تصاویر  نسخه ۸")
    ...
    def _build(self):
        ...
        tk.Label(inner, text="ابزار تغییر اندازه دسته‌ای تصاویر",
                 font=FT, bg=PANEL_BG, fg=TEXT_P).pack(side="left")
        tk.Label(inner, text="  نسخه ۱۰",
    ...
    def _build_images(self, p):
        ...
        b2 = AnimatedButton(row, text="📁  افزودن پوشه", font=FB,
        ...
        # Clear button (danger)
        b3 = AnimatedButton(row, text="✕  پاک کردن همه", font=FB,
                            bg=CARD_BG, fg=ERROR,
    ...
    def _build_crop(self, p):
        ...
        ttk.Checkbutton(ti, text="  ✂  فعال‌سازی برش شکلی (دسته‌ای)",
                        variable=self.crop_on,
                        command=self._tog_crop).pack(side="left")
        tk.Label(ti,
                 text="\nهر تصویر را به شکل انتخاب‌شده برش می‌دهد و ناحیه\n"
                      "بیرون شکل را با رنگ پس‌زمینه پر می‌کند. برای تنظیم دقیق\n"
                      "هر تصویر به‌صورت جداگانه، از تب «ویرایشگر» استفاده کنید.",
        ...
        # Shape selector
        sh = self._card(self.c_frame, "شکل", "🔷")
        ...
        self._sl(self.star_opts, "تعداد پره‌های ستاره", self.star_points, 5, 10, suffix="", cb=self._draw_prev)
        self._sl(self.star_opts, "نسبت داخلی ستاره", self.star_inner, 20, 80, cb=self._draw_prev)

        # Size
        sz = self._card(self.c_frame, "اندازه شکل", "↔")
        self._sl(sz, "پوشش عرض",  self.c_rx, 10,100, cb=self._draw_prev)
        self._sl(sz, "پوشش ارتفاع", self.c_ry, 10,100, cb=self._draw_prev)

        # Offset
        off = self._card(self.c_frame, "جابه‌جایی مرکز  (۰ = مرکز تصویر)", "🎯")
        tk.Label(off, text="منفی = چپ/بالا   مثبت = راست/پایین",
                 font=FS, bg=CARD_BG, fg=TEXT_S).pack(anchor="w", pady=(0,6))
        self._sl(off, "افقی X", self.c_ox, -40,40, cb=self._draw_prev)
        self._sl(off, "عمودی Y",   self.c_oy, -40,40, cb=self._draw_prev)
        AnimatedButton(off, text="↺  بازگشت به مرکز", font=FS,
        ...
        # Preview (thumbnail)
        prev = self._card(self.c_frame, "پیش‌نمایش", "👁")
        ...
        # Presets
        pre = self._card(self.c_frame, "پیش‌تنظیم‌ها", "📐")
        pr = tk.Frame(pre, bg=CARD_BG); pr.pack(anchor="w")
        for nm,rx,ry in [("دایره/مربع",100,100),("عریض",95,75),
                          ("بلند",75,95),("فشرده",80,80)]:
            def _a(rx=rx,ry=ry):
        ...
        # Background color
        bgc = self._card(self.c_frame, "رنگ پس‌زمینه (بیرون شکل)", "🎨")
        ...
        tk.Label(br,text="کد هگز:",font=FS,bg=CARD_BG,fg=TEXT_S).pack(side="left")
        ...
        AnimatedButton(br,text="اعمال",font=FS,bg=BORDER_L,fg=TEXT_S,
    ...
    def _build_trim(self, p):
        ...
        ttk.Checkbutton(ti, text="  ▭  فعال‌سازی حذف خودکار حاشیه (دسته‌ای)",
                        variable=self.trim_on,
                        command=self._tog_trim).pack(side="left")
        tk.Label(ti,
                 text="\nوقتی تصویر را به یک شکل برش می‌دهید، یک حاشیه خالی به رنگ پس‌زمینه اطرافش باقی می‌ماند.\n"
                      "این گزینه آن فضای اضافی را پیدا کرده و پیش از تغییر اندازه حذف می‌کند.\n"
                      "برای رسم دستی کادر برش روی هر تصویر، از تب «ویرایشگر» استفاده کنید.",
                 font=FS, bg=CARD_BG, fg=TEXT_S, justify="left").pack(anchor="w")

        opts_outer = tk.Frame(wrap, bg=CARD_BG,
                       highlightbackground=BORDER_L, highlightthickness=1)
        opts_outer.pack(fill="x", pady=(0,10))
        tk.Label(opts_outer, text="⚙  تنظیمات حذف حاشیه",
                 font=FH, bg=CARD_BG, fg=TEXT_P).pack(
                     anchor="w", padx=14, pady=(10,0))
        opts = tk.Frame(opts_outer, bg=CARD_BG)
        opts.pack(fill="x", padx=14, pady=(4,12))
        self.trim_opts_frame = opts_outer
        self._sl(opts, "حساسیت رنگ", self.trim_tol, 0, 100, suffix="")
        tk.Label(opts, text="حساسیت بالاتر ← رنگ‌های نزدیک به پس‌زمینه هم حذف می‌شوند.",
                 font=FS, bg=CARD_BG, fg=TEXT_S).pack(anchor="w", pady=(0,6))
        self._sl(opts, "حاشیه اضافه", self.trim_pad, 0, 50)
        tk.Label(opts, text="یک حاشیه کوچک دور ناحیه شناسایی‌شده اضافه می‌کند (درصد).",
        ...
        tk.Label(info,
                 text="ℹ  این تب از همان رنگ پس‌زمینه‌ی تب «برش شکلی» استفاده می‌کند.\n"
                      "   ترتیب پردازش: برش شکلی ← حذف حاشیه ← تغییر اندازه ← ذخیره.",
    ...
    def _build_session(self, p):
        ...
        info = self._card(wrap, "ویرایشگر تصویر", "🎨")
        tk.Label(info,
                 text="تصاویر یک پوشه را یکی‌یکی باز کرده و روی هر کدام به‌صورت دستی کار کنید.\n"
                      "دو حالت وجود دارد:\n"
                      "  • برش شکلی — یک شکل انتخاب کنید، آن را روی تصویر رسم و اندازه‌گذاری کنید\n"
                      "  • حذف حاشیه — یک کادر دلخواه بکشید تا فضای اضافه اطراف تصویر حذف شود\n"
                      "بعد از ویرایش همه تصاویر، همه را یکجا ذخیره می‌کنید.",
                 font=FS, bg=CARD_BG, fg=TEXT_S, justify="left").pack(anchor="w", pady=(0,10))

        # Input folder
        r1 = tk.Frame(info, bg=CARD_BG); r1.pack(fill="x", pady=(0,6))
        tk.Label(r1,text="پوشه ورودی:",font=FB,bg=CARD_BG,fg=TEXT_S,
                 width=14,anchor="w").pack(side="left")
        self.ses_in  = tk.StringVar()
        _e1, _f1 = _styled_entry(r1, textvariable=self.ses_in, ipady=5)
        AnimatedButton(r1,text="انتخاب…",font=FS,bg=BORDER_L,fg=TEXT_S,
                  hover_bg=ACCENT, press_bg=ACCENT_H,
                  padx=10,pady=5,
                  command=lambda:self.ses_in.set(
                      filedialog.askdirectory(title="پوشه ورودی") or self.ses_in.get()
                  )).pack(side="left",padx=(6,0))

        # Output folder
        r2 = tk.Frame(info, bg=CARD_BG); r2.pack(fill="x", pady=(0,6))
        tk.Label(r2,text="پوشه خروجی:",font=FB,bg=CARD_BG,fg=TEXT_S,
                 width=14,anchor="w").pack(side="left")
        self.ses_out = tk.StringVar()
        _e2, _f2 = _styled_entry(r2, textvariable=self.ses_out, ipady=5)
        AnimatedButton(r2,text="انتخاب…",font=FS,bg=BORDER_L,fg=TEXT_S,
                  hover_bg=ACCENT, press_bg=ACCENT_H,
                  padx=10,pady=5,
                  command=lambda:self.ses_out.set(
                      filedialog.askdirectory(title="پوشه خروجی") or self.ses_out.get()
                  )).pack(side="left",padx=(6,0))

        # Mode selection
        mr = tk.Frame(info, bg=CARD_BG); mr.pack(fill="x", pady=(6,0))
        tk.Label(mr, text="حالت ویرایش:", font=FB, bg=CARD_BG, fg=TEXT_S,
                 width=14, anchor="w").pack(side="left")
        self.ses_mode = tk.StringVar(value="crop")
        tk.Radiobutton(mr, text="✂  برش شکلی", variable=self.ses_mode, value="crop",
                       bg=CARD_BG, fg=TEXT_S, selectcolor=DARK_BG,
                       activebackground=CARD_BG, activeforeground=ACCENT_L,
                       font=FB, bd=0, cursor="hand2",
                       command=self._tog_session_opts).pack(side="left", padx=(0,16))
        tk.Radiobutton(mr, text="▭  حذف حاشیه", variable=self.ses_mode, value="trim",
                       bg=CARD_BG, fg=TEXT_S, selectcolor=DARK_BG,
                       activebackground=CARD_BG, activeforeground=ACCENT_L,
                       font=FB, bd=0, cursor="hand2",
        ...
        tk.Label(sf,text="شکل پیش‌فرض:",font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w")
        ...
        tk.Label(ef,text="درصد عرض پیش‌فرض:",font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w")
        ...
        tk.Label(eff,text="درصد ارتفاع پیش‌فرض:",font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w")
        ...
        tk.Label(fmf,text="فرمت خروجی:",font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w")
        ...
        tk.Label(rmf,text="حالت تغییر اندازه (پس از ویرایش):",
                 font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w")
        ...
        # Launch button
        AnimatedButton(info, text="🎨  باز کردن ویرایشگر",
                  font=(_PFONT,13,"bold"),
                  bg=ACCENT, fg=TEXT_P,
                  hover_bg=ACCENT_L, press_bg=ACCENT_H,
                  padx=24, pady=12,
        ...
        tk.Label(info,
                 text="ℹ  ویرایشگر مستقل است — پوشه‌های ورودی/خروجی مخصوص به خود را دارد،\n"
                      "   از لیست فایل‌های تب «تصاویر» استفاده نمی‌کند، و از تنظیمات حالت تغییر اندازه /\n"
                      "   ابعاد / فرمت خروجی / اندازه فایل که زیر آورده شده استفاده می‌کند.",
    ...
    def _build_resize(self, p):
        ...
        fc = self._card(wrap, "فرمت خروجی", "📄")
        tk.Label(fc,
                 text="تصاویر را در فرمت انتخاب‌شده ذخیره می‌کند. پسوند فایل هم به‌طور خودکار تغییر می‌کند.",
                 font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w",pady=(0,6))
        ...
        tk.Label(fc,text="⚠ فرمت‌های JPEG و BMP شفافیت ندارند (کانال آلفا حذف می‌شود).",
                 font=FS,bg=CARD_BG,fg=WARNING).pack(anchor="w",pady=(4,0))

        px = self._card(wrap,"ابعاد هدف (پیکسل)","↔")
        ...
        def _pf(p,lbl,var):
            ...
            tk.Label(f,text="پیکسل",font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="e")
        _pf(pr,"عرض",self.width_var)
        tk.Label(pr,text="×",font=(_PFONT,16),
                 bg=CARD_BG,fg=TEXT_S).pack(side="left",padx=4,pady=(14,0))
        _pf(pr,"ارتفاع",self.height_var)

        mc = self._card(wrap,"حالت تغییر اندازه (دسته‌ای)","⚙")
        modes=[("exact","دقیق","کشیدن به W×H"),
               ("fit","تناسب","حفظ نسبت ابعاد"),
               ("fill","پر کردن","پر کردن + برش از مرکز"),
               ("width_only","فقط عرض","بر اساس عرض"),
               ("height_only","فقط ارتفاع","بر اساس ارتفاع"),
               ("no_resize","بدون تغییر اندازه","ذخیره بدون تغییر")]
        ...
        tc=self._card(wrap,"سرعت پردازش","⚡")
        tk.Label(tc,text="تعداد پردازش‌های موازی (بیشتر = سریع‌تر):",font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w")
        ...
        ttk.Checkbutton(sh,text="  📦  محدود کردن اندازه فایل خروجی",
        ...
        tk.Label(self.sz_f,text="هر تصویر را زیر یک اندازه مشخص فشرده می‌کند.",
                 font=FS,bg=CARD_BG,fg=TEXT_S).pack(anchor="w")
        tk.Frame(self.sz_f,bg=BORDER_L,height=1).pack(fill="x",pady=4)
        sr=tk.Frame(self.sz_f,bg=CARD_BG); sr.pack(anchor="w")
        tk.Label(sr,text="حداکثر اندازه:",font=FB,bg=CARD_BG,fg=TEXT_S).pack(side="left",padx=(0,6))
        self.sz_e, _szf = _styled_entry(sr, textvariable=self.size_kb_var, width=7,
                         font=(_PFONT,13,"bold"), justify="center",
                         ipady=5, fill=None, expand=False)
        self.kb_btn=AnimatedButton(sr,text="کیلوبایت",font=FB,bg=ACCENT,fg=TEXT_P,
                               hover_bg=ACCENT_L, press_bg=ACCENT_H,
                               padx=8,pady=5,
                               command=lambda:self._set_unit("KB"))
        self.kb_btn.pack(side="left",padx=(6,2))
        self.mb_btn=AnimatedButton(sr,text="مگابایت",font=FB,bg=BORDER_L,fg=TEXT_S,
    ...
    def _build_bottom(self, parent):
        ...
        self.start_btn=AnimatedButton(right,
            text="▶  شروع پردازش دسته‌ای",font=FH,
            bg=ACCENT,fg=TEXT_P,
            hover_bg=ACCENT_L, press_bg=ACCENT_H,
            padx=24,pady=12,
            command=self._start)
    ...
    def _apply_bg(self):
        ...
        try:
            ...
            messagebox.showerror("خطای رنگ","یک کد هگز معتبر وارد کنید، مثلاً #ffffff")

    # ── File management ───────────────────────────────────────────────────────
    def _add_files(self):
        fs=filedialog.askopenfilenames(
            title="انتخاب تصاویر",
            filetypes=[("تصاویر","*.jpg *.jpeg *.png *.webp *.bmp *.tiff *.tif"),
                       ("همه فایل‌ها","*.*")])
        for f in fs:
    ...
    def _ref_list(self):
        ...
        n=len(self.files)
        self.fc_lbl.config(
            text=f"{n} تصویر انتخاب شده",
            fg=SUCCESS if n>0 else TEXT_S)

    def _pick_out(self):
        d=filedialog.askdirectory(title="پوشه خروجی")
    ...
    def _launch_session(self):
        ...
        if not in_dir:
            messagebox.showwarning("بدون پوشه","لطفاً یک پوشه ورودی انتخاب کنید.")
            return
        if not out_dir:
            messagebox.showwarning("بدون خروجی","لطفاً یک پوشه خروجی انتخاب کنید.")
            return
        exts={".jpg",".jpeg",".png",".webp",".bmp",".tiff",".tif"}
        files=[str(p) for p in sorted(Path(in_dir).iterdir())
               if p.suffix.lower() in exts]
        if not files:
            messagebox.showwarning("بدون تصویر","هیچ تصویری در آن پوشه پیدا نشد.")
            return
    ...
    def _start(self):
        ...
        if not self.files:
            messagebox.showwarning("بدون تصویر","حداقل یک تصویر اضافه کنید."); return
        out=self.output_dir.get().strip()
        if not out:
            messagebox.showwarning("بدون خروجی","یک پوشه خروجی انتخاب کنید."); return
        tw=self._get_dim(self.width_var,413)
        th=self._get_dim(self.height_var,531)
        if self.mode_var.get()!="no_resize" and (tw<=0 or th<=0):
            messagebox.showerror("اندازه نامعتبر","مقادیر پیکسل معتبر وارد کنید."); return
        target_kb=self._get_target_kb()
        ...
        self.prog_var.set(0); self.pct_lbl.config(text="۰٪")
    ...
    def _show_report(self, results, elapsed, out_dir):
        ...
        try:
            ...
            summary = (f"✔ {len(ok_list)}/{len(results)} موفق  ·  "
                       f"✗ {len(err_list)} خطا  ·  {elapsed:.2f} ثانیه  ·  "
                       f"{total_kb/1024:.1f} مگابایت خروجی")
            ...
            tk.Button(btns, text="بستن", bg="#166534", fg="#ecfdf5", relief="flat",
                      command=win.destroy).pack(side="left", padx=3)
            if out_dir:
                tk.Button(btns, text="باز کردن پوشه خروجی", bg="#10b981", fg="#061a10",
    ...
    def _open_folder(self, folder):
        """پوشه را در سیستم‌عامل باز می‌کند."""
        try:
            import os, sys
            if os.name == "nt":
    ...
    def _build_voice(self, p):
        ...
        if not VOSK_AVAILABLE:
            missing.append("vosk (برای حالت آفلاین)   →   pip install vosk")
        if missing:
            warn = self._card(wrap, "کتابخانه‌های لازم نصب نشده‌اند", "⚠")
            for m in missing:
                tk.Label(warn, text="•  "+m, font=FM, bg=CARD_BG, fg=WARNING,
                         justify="left").pack(anchor="w")
            tk.Label(warn, text="پس از نصب پکیج‌های بالا، برنامه را دوباره اجرا کنید.",
                     font=FS, bg=CARD_BG, fg=TEXT_S).pack(anchor="w", pady=(6,0))

        # Intro
        intro = self._card(wrap, "گفتار به متن  ←  Word / Excel / PowerPoint", "🎙")
        tk.Label(intro,
                 text="در میکروفون صحبت کنید؛ متن به‌صورت زنده تایپ می‌شود.\n"
                      "بعداً می‌توانید متن را ویرایش کرده و آن را به فایل Word، Excel یا PowerPoint خروجی بگیرید.",
                 font=FS, bg=CARD_BG, fg=TEXT_S, justify="left").pack(anchor="w")

        # Engine (online / offline)
        ec2 = self._card(wrap, "حالت تشخیص گفتار", "🔌")
        erow = tk.Frame(ec2, bg=CARD_BG); erow.pack(anchor="w")
        tk.Radiobutton(erow, text="آنلاین (گوگل) — نیاز به اینترنت دارد", font=FB,
        ...
        tk.Radiobutton(erow, text="آفلاین (Vosk) — نیازی به اینترنت ندارد", font=FB,
                       bg=CARD_BG, fg=TEXT_P, selectcolor=DARK_BG,
                       activebackground=CARD_BG, activeforeground=ACCENT_L,
        ...
        tk.Label(self.voice_offline_opts,
                 text="پوشه مدل Vosk را انتخاب کنید (مدل زبان مورد نظر را از\n"
                      "alphacephei.com/vosk/models دانلود و از حالت فشرده خارج کنید):",
        ...
        AnimatedButton(mrow, text="انتخاب پوشه…", font=FB, bg=CARD_BG, fg=TEXT_P,
                  hover_bg=_lighten(CARD_BG,20), press_bg=_darken(CARD_BG,10),
                  padx=10, pady=4,
        ...
        # Language
        lc = self._card(wrap, "زبان تشخیص گفتار", "🌐")
        row = tk.Frame(lc, bg=CARD_BG); row.pack(anchor="w")
        tk.Label(row, text="زبان:", font=FB, bg=CARD_BG, fg=TEXT_S).pack(side="left", padx=(0,8))
        ...
        AnimatedButton(btnrow, text="↵  خط جدید", font=FB, bg=CARD_BG, fg=TEXT_S,
                  hover_bg=_lighten(CARD_BG,20), press_bg=_darken(CARD_BG,10),
                  padx=12, pady=8,
                  command=self._voice_next_line).pack(side="left", padx=(0,8))

        AnimatedButton(btnrow, text="⌫  حذف آخرین خط", font=FB, bg=CARD_BG, fg=TEXT_S,
                  hover_bg=_lighten(CARD_BG,20), press_bg=_darken(CARD_BG,10),
                  padx=12, pady=8,
                  command=self._voice_undo_last).pack(side="left", padx=(0,8))

        AnimatedButton(btnrow, text="🗑  پاک کردن همه", font=FB, bg=CARD_BG, fg=ERROR,
                  hover_bg=_mix(CARD_BG, ERROR, 0.15), press_bg=_mix(CARD_BG, ERROR, 0.25),
                  padx=12, pady=8,
                  command=self._voice_clear).pack(side="left")

        self.voice_status_lbl = tk.Label(cc, text="◼  آماده", font=FS, bg=CARD_BG, fg=TEXT_S)
        self.voice_status_lbl.pack(anchor="w")

        # Text preview
        pc = self._card(wrap, "پیش‌نمایش متن", "👁")
        tk.Label(pc,
                 text="هر خط در این کادر هنگام خروجی‌گیری به‌عنوان یک «سطر» جداگانه در نظر گرفته می‌شود.",
        ...
        # Output
        ec = self._card(wrap, "خروجی", "📤")
        fr = tk.Frame(ec, bg=CARD_BG); fr.pack(anchor="w")
        tk.Label(fr, text="فرمت خروجی:", font=FB, bg=CARD_BG, fg=TEXT_S).pack(side="left", padx=(0,10))
        for val, lbl in [("xlsx","📊  اکسل"), ("docx","📄  ورد"), ("pptx","📽  پاورپوینت")]:
        ...
        tk.Label(self.voice_pptx_opts, text="چیدمان پاورپوینت:", font=FS,
                 bg=CARD_BG, fg=TEXT_S).pack(anchor="w", pady=(8,2))
        tk.Radiobutton(self.voice_pptx_opts,
                       text="همه سطرها به‌صورت بولت در یک اسلاید",
                       variable=self.voice_pptx_layout, value="bullets",
                       bg=CARD_BG, fg=TEXT_S, selectcolor=DARK_BG,
                       activebackground=CARD_BG, activeforeground=ACCENT_L,
                       font=FB, bd=0, cursor="hand2").pack(anchor="w")
        tk.Radiobutton(self.voice_pptx_opts,
                       text="هر سطر = یک اسلاید جداگانه (عنوان اسلاید)",
        ...
        tk.Label(pr2, text="مسیر فایل خروجی:", font=FB, bg=CARD_BG, fg=TEXT_S,
                 width=14, anchor="w").pack(side="left")
        _styled_entry(pr2, textvariable=self.voice_out_path, ipady=5)
        AnimatedButton(pr2, text="انتخاب…", font=FS, bg=BORDER_L, fg=TEXT_S,
                  hover_bg=ACCENT, press_bg=ACCENT_H,
                  padx=10, pady=5,
                  command=self._voice_pick_output).pack(side="left", padx=(6,0))

        AnimatedButton(ec, text="💾  ذخیره خروجی", font=(_PFONT,13,"bold"),
                  bg=ACCENT, fg=TEXT_P,
                  hover_bg=ACCENT_L, press_bg=ACCENT_H,
                  padx=24, pady=12,
    ...
    def _voice_pick_model_dir(self):
        d = filedialog.askdirectory(title="انتخاب پوشه مدل Vosk")
        if d:
    ...
    def _voice_start(self):
        if not SR_AVAILABLE:
            messagebox.showerror(
                ...
                "برای استفاده از تشخیص گفتار، دستور زیر را در ترمینال اجرا کنید:\n\n"
        ...
        if engine == "offline":
            if not VOSK_AVAILABLE:
                messagebox.showerror(
                    "کتابخانه در دسترس نیست",
                    "برای استفاده از حالت آفلاین، دستور زیر را در ترمینال اجرا کنید:\n\n"
                    "pip install vosk")
                return
            ...
            if not model_path or not os.path.isdir(model_path):
                messagebox.showwarning(
                    "مدلی انتخاب نشده",
                    "ابتدا پوشه مدل Vosk را انتخاب کنید "
                    "(یکی را از alphacephei.com/vosk/models دانلود کنید).")
                return
            try:
                if self.vosk_model is None or self.vosk_model_loaded_path != model_path:
                    self._voice_set_status("◌  در حال بارگذاری مدل آفلاین…", ACCENT)
                    self.root.update_idletasks()
                    self.vosk_model = vosk.Model(model_path)
                    self.vosk_model_loaded_path = model_path
                offline_model = self.vosk_model
            except Exception as e:
                messagebox.showerror("خطای مدل آفلاین", f"بارگذاری مدل ناموفق بود:\n{e}")
                self._voice_set_status("◼  خطای مدل آفلاین", ERROR)
                return
        ...
        try:
            ...
            self._voice_set_status("◌  در حال کالیبره کردن میکروفون…", ACCENT)
            with self.voice_mic as source:
                self.voice_recognizer.adjust_for_ambient_noise(source, duration=0.6)
        except Exception as e:
            messagebox.showerror("خطای میکروفون",
                                  f"دسترسی به میکروفون ممکن نشد:\n{e}")
            self._voice_set_status("◼  خطای میکروفون", ERROR)
            return
        ...
        try:
            ...
            messagebox.showerror("خطا", f"شروع ضبط ممکن نشد:\n{e}")
            return

        self.voice_listening = True
        self.start_voice_btn.config(state="disabled")
        self.stop_voice_btn.config(state="normal")
        self._voice_set_status("●  در حال گوش دادن… الان صحبت کنید", SUCCESS)
    ...
    def _voice_stop(self):
        ...
        self._voice_set_status("◼  متوقف شد", TEXT_S)
    ...
    def _voice_clear(self):
        if not self.voice_text.get("1.0", "end-1c").strip():
            return
        if messagebox.askyesno("پاک کردن", "همه متن دیکته‌شده پاک شود؟"):
            self.voice_text.delete("1.0", "end")
            self._voice_set_status("◼  آماده", TEXT_S)
    ...
    def _voice_pick_output(self):
        ...
        types = {"xlsx":[("کارپوشه اکسل","*.xlsx")],
                 "docx":[("سند ورد","*.docx")],
                 "pptx":[("ارائه پاورپوینت","*.pptx")]}[fmt]
        f = filedialog.asksaveasfilename(
            title="ذخیره خروجی", defaultextension=ext,
            filetypes=types + [("همه فایل‌ها","*.*")])
        if f:
    ...
    def _voice_export(self):
        ...
        if not lines:
            messagebox.showwarning("بدون متن",
                                    "ابتدا در میکروفون صحبت کنید یا متنی در کادر بنویسید.")
            return
        ...
        try:
            ...
            self._voice_set_status(f"✗  خطا: {e}", ERROR)
            return

        self._voice_set_status(f"✔  ذخیره شد: {Path(out).name}", SUCCESS)
        messagebox.showinfo("انجام شد", f"فایل با موفقیت ذخیره شد:\n{out}")
